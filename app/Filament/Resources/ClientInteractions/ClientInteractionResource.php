...
class ClientInteractionResource extends Resource
{
        ...
        return $schema->components([
            Section::make('تسجيل متابعة')->columns(2)->schema([
                ...
                Select::make('employee_id')->label('الموظف')
                    ->options(fn (): array => User::query()->employees()
                        ->where('is_active', true)
                        ->orderBy('name')
                        ->get()
                        ->mapWithKeys(fn (User $employee): array => [
                            $employee->id => $employee->job_title ? $employee->name.' — '.$employee->job_title : $employee->name,
                        ])->all())
                ...
                SpatieMediaLibraryFileUpload::make('attachments')->label('المرفقات')->collection('attachments')->multiple()
                    ->acceptedFileTypes(['application/pdf', 'image/jpeg', 'image/png', 'image/webp'])->maxSize(10240)->downloadable()->openable()->columnSpanFull(),
            ]),
        ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('contact_method')->label('الوسيلة')->badge()->formatStateUsing(fn ($state): string => $state instanceof ContactMethod ? $state->label() : (ContactMethod::tryFrom($state)?->label() ?? '—')),
            ...
            SelectFilter::make('employee_id')
                ->label('الموظف')
                ->options(fn (): array => User::query()->employees()->orderBy('name')->pluck('name', 'id')->all()),
