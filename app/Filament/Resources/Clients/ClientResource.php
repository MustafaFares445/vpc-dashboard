            ...
            Section::make('معلومات العميل')
                ...
                ->schema([
                    ...
                    Select::make('assigned_to')
                        ...
                        ->searchable()->preload()->visible(fn (): bool => auth()->user()->isAdmin()),
                    ...
                    SpatieMediaLibraryFileUpload::make('attachments')
                        ->label('المرفقات')->collection('attachments')->multiple()
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('next_follow_up_at')->label('المتابعة القادمة')->dateTime()->placeholder('—')->sortable()
                    ->color(fn (Client $record): ?string => $record->next_follow_up_at?->isPast() ? 'danger' : null),
            ])
            ->filters([
                SelectFilter::make('status')->label('الحالة')->options(ClientStatus::options()),
                SelectFilter::make('assigned_to')->label('الموظف')->relationship('assignedUser', 'name')->visible(fn (): bool => auth()->user()->isAdmin()),
